...
Node* SearchingAVL(Node* root,int key){
 if (root==NULL)
 {
 return NULL;
 }
 ...
 if (root->key>key)
 {
 return SearchingAVL(root->left,key);
 }
 else{
 return SearchingAVL(root->right,key); 
 }
